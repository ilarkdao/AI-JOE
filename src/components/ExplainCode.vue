    ...
    <div  class="exchange">
        ...
        <div class="round-box" v-if="clickFlag">		
					<div class="round-box-content-container" style="margin-top: -0.5rem;">	
						<b-form-textarea
							v-model="prompt"
							placeholder="请输入代码, 不宜超过500字"
							rows="8"
							max-rows="6"
						></b-form-textarea>		
					</div>
        </div>
				<b-button  block pill variant="outline-primary" @click="clickWords" v-if="clickFlag">解释代码</b-button>
    ...
    methods: {
			async clickWords(){
			  ...
			  try {
					...
					this.value = '\`\`\` \n'+this.prompt+'\n\`\`\`\n\n'
					// console.log(255, prompt, "value", this.value)
